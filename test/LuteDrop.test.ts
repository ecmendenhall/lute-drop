...
describe("LuteDrop", () => {
  ...
  describe("configuration", () => {
    ...
    it("stores the Loot contract", async () => {
      expect(await contracts.luteDrop.loot()).to.equal(contracts.loot.address);
    });
    ...
    it("stores the loot claimable supply", async () => {
      expect(await contracts.luteDrop.lootClaimableSupply()).to.equal(2);
    });

    it("stores the mLoot claimable supply", async () => {
      expect(await contracts.luteDrop.mlootClaimableSupply()).to.equal(4);
    });
  });
  ...
  describe("minting", () => {
    ...
    describe("loot holders", () => {
      ...
      it("reverts on invalid items", async () => {
        await expect(
          contracts.luteDrop.connect(lootHolder1).claim(3, 0, 0)
        ).to.be.revertedWith(
      ...
      it("reverts if supply is fully claimed", async () => {
        ...
        await expect(
          contracts.luteDrop.connect(lootWhale).claim(0, 0, 2)
        ).to.be.revertedWith("Loot holder supply fully claimed");
      });

      it("loot holders can claim only one item per token", async () => {
        await contracts.luteDrop.connect(lootWhale).claim(0, 0, 2);
        expect(
          contracts.luteDrop.connect(lootWhale).claim(1, 0, 2)
        ).to.be.revertedWith("Item already claimed");
      });

      it("loot holders cannot claim for unowned tokens", async () => {
        await expect(
          contracts.luteDrop.connect(lootWhale).claim(0, 0, 0)
        ).to.be.revertedWith("Must hold specified token to claim");
      });
    });
    ...
    describe("mloot holders", () => {
      ...
      it("reverts on invalid items", async () => {
        await expect(
          contracts.luteDrop.connect(mlootHolder1).claim(3, 1, 0)
        ).to.be.revertedWith(
          "Transaction reverted: function was called with incorrect parameters"
        );
      });
      ...
      it("reverts if supply is fully claimed", async () => {
        ...
        await expect(
          contracts.luteDrop.connect(mlootWhale).claim(0, 1, 4)
        ).to.be.revertedWith("mLoot holder supply fully claimed");
      });

      it("mloot holders can claim only one item per token", async () => {
        await contracts.luteDrop.connect(mlootWhale).claim(0, 1, 2);
        expect(
          contracts.luteDrop.connect(mlootWhale).claim(1, 1, 2)
        ).to.be.revertedWith("Item already claimed");
        await contracts.luteDrop.connect(mlootWhale).claim(0, 1, 3);
      ...
      it("mloot holders cannot claim for unowned tokens", async () => {
        await expect(
          contracts.luteDrop.connect(mlootWhale).claim(0, 1, 0)
        ).to.be.revertedWith("Must hold specified token to claim");
      });
    });
  });
